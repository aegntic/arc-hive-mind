...
export default defineConfig(({ mode }: ConfigEnv): UserConfig => {
  ...
  const internalHost = 'archivemind-server';  // Docker service name for internal communication
  const externalHost = process.env.HOST || 'localhost';  // Host for external access
  const host = isDocker ? internalHost : externalHost;
  const port = process.env.ARCHIVEMIND_SERVER_PORT || env.ARCHIVEMIND_SERVER_PORT || '8181';
  ...
  return {
    ...
    server: {
      ...
      port: parseInt(process.env.ARCHIVEMIND_UI_PORT || env.ARCHIVEMIND_UI_PORT || '3737'), // Use configurable port
