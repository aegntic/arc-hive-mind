...
export const MCPPage = () => {
  ...
  const loadConfiguration = async () => {
    try {
      ...
      const defaultPort = import.meta.env.ARCHIVEMIND_MCP_PORT || 8051;
      setConfig({
        transport: 'http',
        host: 'localhost',
  ...
  const generateCursorDeeplink = () => {
    ...
    return `cursor://anysphere.cursor-deeplink/mcp/install?name=archivemind&config=${base64Config}`;
  };

  const handleCursorOneClick = () => {
    const deeplink = generateCursorDeeplink();
    window.location.href = deeplink;
    showToast('Opening Cursor with Archivemind MCP configuration...', 'info');
  };
      ...
      case 'claudecode':
        return {
          ...
          steps: [
            ...
            `2. claude mcp add --transport http archivemind http://${config?.host}:${config?.port}/mcp`,
      ...
      {activeTab === 'server' && (
        <>
          ...
          <motion.div className="grid grid-cols-1 gap-6" variants={itemVariants}>
            ...
            {/* Left Column: Archivemind MCP Server */}
                  ...
                  <div className="border-t border-gray-200 dark:border-zinc-800 pt-4">
                    ...
                    <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                      <p className="text-xs text-blue-700 dark:text-blue-300">
                        <span className="font-semibold">Note:</span> Archivemind works with any application that supports MCP. 
