...
class MCPService {
  ...
  /**
   * Get available tools from the running MCP server (legacy - for Archivemind default client)
   */
  ...
  /**
   * Call a specific MCP tool (legacy - for Archivemind default client)
   */
  ...
  async getAllAvailableTools(): Promise<{
    ...
    try {
      // Get Archivemind tools (default client)
      const archivemindTools = await this.getAvailableTools();
      ...
      const totalCount = archivemindTools.length + clientTools.reduce((sum, ct) => sum + ct.tools.length, 0);

      return {
        archivemind_tools: archivemindTools,
