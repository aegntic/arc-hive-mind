...
class MCPClientService {
  ...
  /**
   * Get tools from all connected clients (including Archivemind via MCP client)
   */
  ...
  // ========================================
  // ARCHIVEMIND INTEGRATION HELPERS
  // ========================================

  /**
   * Create Archivemind MCP client using Streamable HTTP transport
   */
  async createArchivemindClient(): Promise<MCPClient> {
    // Require ARCHIVEMIND_MCP_PORT to be set
    const mcpPort = import.meta.env.ARCHIVEMIND_MCP_PORT;
    if (!mcpPort) {
      throw new Error(
    ...
    const archivemindConfig: MCPClientConfig = {
      name: 'Archivemind',
      transport_type: 'http',
      connection_config: {
        url: mcpUrl
    ...
    return this.createClient(archivemindConfig);
  }

  /**
   * Get the default Archivemind client (or create if doesn't exist)
   */
  async getOrCreateArchivemindClient(): Promise<MCPClient> {
    const clients = await this.getClients();
    const archivemindClient = clients.find(client => client.is_default || client.name === 'Archivemind');

    if (archivemindClient) {
      return archivemindClient;
    }

    return this.createArchivemindClient();
  }
}
