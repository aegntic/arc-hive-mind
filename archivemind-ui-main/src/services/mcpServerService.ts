...
/**
 * MCP Server Service - Handles the Archivemind MCP server lifecycle via FastAPI
 */
class MCPServerService {
  ...
  // ========================================
  // LEGACY ARCHIVEMIND TOOL ACCESS (For backward compatibility)
  // ========================================

  /**
   * Make an MCP call to the running Archivemind server via SSE
   */
  ...
  /**
   * Get available tools from the running Archivemind MCP server
   * @deprecated Use mcpClientService for tool discovery instead
   */
  async getAvailableTools(): Promise<MCPTool[]> {
    try {
      console.log('Attempting direct MCP tools/list call to Archivemind server...');
      const result = await this.makeMCPCall('tools/list');
      const validatedResult = MCPToolsListResponseSchema.parse(result);
      console.log('Successfully retrieved tools from Archivemind server:', validatedResult.tools.length);
      return validatedResult.tools;
    } catch (mcpError) {
      console.warn('Direct MCP call to Archivemind server failed:', mcpError);
      throw new Error(`Failed to retrieve tools from Archivemind server: ${mcpError instanceof Error ? mcpError.message : mcpError}`);
    }
  }

  /**
   * Call a specific tool on the Archivemind MCP server
  ...
  async callTool(name: string, arguments_: Record<string, any>): Promise<any> {
    try {
      ...
      console.error(`Failed to call Archivemind MCP tool ${name}:`, error);
      throw error;
    }
  }
}
