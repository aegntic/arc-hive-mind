...
export const ClientCard = ({
  ...
  // Special styling for Archivemind client
  const isArchivemindClient = client.name.includes('Archivemind') || client.name.includes('archivemind');

  // Status-based styling
  const statusConfig = {
    online: {
      color: isArchivemindClient ? 'archivemind' : 'cyan',
      glow: isArchivemindClient ? 'shadow-[0_0_25px_rgba(59,130,246,0.7),0_0_15px_rgba(168,85,247,0.5)] dark:shadow-[0_0_35px_rgba(59,130,246,0.8),0_0_20px_rgba(168,85,247,0.7)]' : 'shadow-[0_0_15px_rgba(34,211,238,0.5)] dark:shadow-[0_0_20px_rgba(34,211,238,0.7)]',
      border: isArchivemindClient ? 'border-blue-400/60 dark:border-blue-500/60' : 'border-cyan-400/50 dark:border-cyan-500/40',
      badge: isArchivemindClient ? 'bg-blue-500/30 text-blue-400 border-blue-500/40' : 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30',
      pulse: isArchivemindClient ? 'bg-blue-400' : 'bg-cyan-400'
    },
    offline: {
      color: 'gray',
  ...
  useEffect(() => {
    if (!isArchivemindClient || !particlesRef.current) return;
  ...
  // Special background for Archivemind client
  const archivemindBackground = isArchivemindClient ? 'bg-gradient-to-b from-white/80 via-blue-50/30 to-white/60 dark:from-white/10 dark:via-blue-900/10 dark:to-black/30' : 'bg-gradient-to-b from-white/80 to-white/60 dark:from-white/10 dark:to-black/30';

  return (
    <div 
      className={`flip-card h-[220px] cursor-pointer ${isArchivemindClient ? 'order-first' : ''}`} 
    ...
    >
      <div className={`relative w-full h-full transition-all duration-500 transform-style-preserve-3d ${isFlipped ? 'rotate-y-180' : ''} ${isHovered && !isFlipped ? 'hover-lift' : ''}`}>
        {/* Front Side */}
        <div 
          className={`absolute w-full h-full backface-hidden backdrop-blur-md ${archivemindBackground} rounded-xl p-5 ${currentStatus.border} ${currentStatus.glow} transition-all duration-300 ${isArchivemindClient ? 'archivemind-card-border overflow-hidden' : ''}`} 
          ref={isArchivemindClient ? particlesRef : undefined}
        >
          {/* Particle container for Archivemind client */}
          {isArchivemindClient && (
          ...
          {/* Subtle aurora glow effect for Archivemind client */}
          {isArchivemindClient && (
          ...
          {onEdit && (
            <button 
              onClick={handleEdit} 
              className={`absolute top-3 right-12 p-1.5 rounded-full ${isArchivemindClient ? 'bg-blue-200/50 dark:bg-blue-900/50 hover:bg-blue-300/50 dark:hover:bg-blue-800/50' : 'bg-gray-200/50 dark:bg-gray-800/50 hover:bg-gray-300/50 dark:hover:bg-gray-700/50'} transition-colors transform hover:scale-110 transition-transform duration-200 z-20`} 
              title="Edit client configuration"
            >
              <Settings className={`w-4 h-4 ${isArchivemindClient ? 'text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400'}`} />
            </button>
          )}

          {/* Delete button - only for non-Archivemind clients */}
          {!isArchivemindClient && onDelete && (
            <button 
              onClick={(e) => {
                e.stopPropagation();
                onDelete(client);
          ...
          <div className="flex items-start">
            {isArchivemindClient ? (
              <div className="p-3 rounded-lg bg-gradient-to-br from-blue-500/20 to-purple-500/20 mr-3 relative pulse-soft">
                <img src="/logo-neon.png" alt="Archivemind" className="w-6 h-6 drop-shadow-[0_0_8px_rgba(59,130,246,0.8)] animate-glow-pulse" />
            ...
            <div>
              <h3 className={`font-bold text-gray-800 dark:text-white text-lg ${isArchivemindClient ? 'bg-gradient-to-r from-blue-400 to-purple-500 text-transparent bg-clip-text animate-text-shimmer' : ''}`}>
                {client.name}
              </h3>
          ...
          <div className="mt-5 space-y-2">
            ...
            <div className="flex items-center text-sm">
              ...
              <span className={`text-gray-600 dark:text-gray-400 ml-auto ${isArchivemindClient ? 'font-medium text-blue-600 dark:text-blue-400' : ''}`}>
                {client.version}
              </span>
            </div>
            <div className="flex items-center text-sm">
          ...
          <button 
            onClick={toggleFlip} 
            className={`absolute bottom-4 right-4 p-1.5 rounded-full ${isArchivemindClient ? 'bg-blue-200/50 dark:bg-blue-900/50 hover:bg-blue-300/50 dark:hover:bg-blue-800/50' : 'bg-gray-200/50 dark:bg-gray-800/50 hover:bg-gray-300/50 dark:hover:bg-gray-700/50'} transition-colors transform hover:scale-110 transition-transform duration-200 z-10`} 
            title="View available tools"
          >
            <Hammer className={`w-4 h-4 ${isArchivemindClient ? 'text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400'}`} />
          </button>
        </div>

        {/* Back Side */}
        <div className={`absolute w-full h-full backface-hidden backdrop-blur-md ${archivemindBackground} rounded-xl p-5 rotate-y-180 ${currentStatus.border} ${currentStatus.glow} transition-all duration-300 ${isArchivemindClient ? 'archivemind-card-border' : ''}`}>
          {/* Subtle aurora glow effect for Archivemind client */}
          {isArchivemindClient && (
            <div className="absolute inset-0 rounded-xl overflow-hidden opacity-20">
              <div className="absolute -inset-[100px] bg-[radial-gradient(circle,rgba(59,130,246,0.8)_0%,rgba(168,85,247,0.6)_40%,transparent_70%)] blur-3xl animate-[pulse_8s_ease-in-out_infinite]"></div>
          ...
          {/* Delete button on back side - only for non-Archivemind clients */}
          {!isArchivemindClient && onDelete && (
            <button 
              onClick={(e) => {
                e.stopPropagation();
          ...
          <h3 className={`font-bold text-gray-800 dark:text-white mb-3 flex items-center ${isArchivemindClient ? 'bg-gradient-to-r from-blue-400 to-purple-500 text-transparent bg-clip-text animate-text-shimmer' : ''}`}>
            <Hammer className={`w-4 h-4 mr-2 ${isArchivemindClient ? 'text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400'}`} />
          ...
          <div className="space-y-2 overflow-y-auto max-h-[140px] pr-1 hide-scrollbar">
            {client.tools.length === 0 ? (
              ...
              client.tools.map(tool => (
                <div 
                  key={tool.id} 
                  className={`p-2 rounded-md ${isArchivemindClient ? 'bg-blue-50/50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700/50 hover:border-blue-300 dark:hover:border-blue-600/50' : 'bg-gray-100/50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 hover:border-gray-300 dark:hover:border-gray-600/50'} transition-colors transform hover:translate-x-1 transition-transform duration-200`}
                >
                  <div className="flex items-center justify-between">
                    <span className={`font-mono text-xs ${isArchivemindClient ? 'text-blue-600 dark:text-blue-400' : 'text-blue-600 dark:text-blue-400'}`}>
                      {tool.name}
                    </span>
          ...
          >
            <Server className={`w-4 h-4 ${isArchivemindClient ? 'text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400'}`} />
          </button>
        </div>
      </div>
    </div>
