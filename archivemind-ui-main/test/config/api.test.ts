...
describe('API Configuration', () => {
  ...
  describe('getApiUrl', () => {
    ...
    it('should use default port 8181 when no port environment variables are set in development', async () => {
      ...
      delete (import.meta.env as any).VITE_ARCHIVEMIND_SERVER_PORT;
      delete (import.meta.env as any).VITE_PORT;
      delete (import.meta.env as any).ARCHIVEMIND_SERVER_PORT;
    ...
    it('should use VITE_ARCHIVEMIND_SERVER_PORT when set in development', async () => {
      // Development mode with custom port via VITE_ prefix
      delete (import.meta.env as any).PROD;
      delete (import.meta.env as any).VITE_API_URL;
      (import.meta.env as any).VITE_ARCHIVEMIND_SERVER_PORT = '9191';
    ...
    it('should use custom port with https protocol', async () => {
      ...
      delete (import.meta.env as any).PROD;
      delete (import.meta.env as any).VITE_API_URL;
      (import.meta.env as any).VITE_ARCHIVEMIND_SERVER_PORT = '8443';
  ...
  describe('Port validation', () => {
    it('should handle various port formats', async () => {
      ...
      for (const { port, expected } of testCases) {
        ...
        delete (import.meta.env as any).PROD;
        delete (import.meta.env as any).VITE_API_URL;
        (import.meta.env as any).VITE_ARCHIVEMIND_SERVER_PORT = port;
...
describe('MCP Client Service Configuration', () => {
  ...
  it('should throw error when ARCHIVEMIND_MCP_PORT is not set', async () => {
    delete (import.meta.env as any).ARCHIVEMIND_MCP_PORT;
    
    const { mcpClientService } = await import('../../src/services/mcpClientService');
    
    await expect(mcpClientService.createArchivemindClient()).rejects.toThrow('ARCHIVEMIND_MCP_PORT environment variable is required');
    await expect(mcpClientService.createArchivemindClient()).rejects.toThrow('Default value: 8051');
  });

  it('should use ARCHIVEMIND_MCP_PORT when set', async () => {
    (import.meta.env as any).ARCHIVEMIND_MCP_PORT = '9051';
    (import.meta.env as any).ARCHIVEMIND_SERVER_PORT = '8181';
    ...
    try {
      await mcpClientService.createArchivemindClient();
