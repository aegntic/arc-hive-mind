---
title: Configuration
description: Essential configuration for Archivemind
sidebar_position: 3
---
...
Archivemind uses a microservices architecture with three separate services. All services run in Docker containers and communicate via the `app-network` bridge network using service names, not localhost. Real-time features use Socket.IO for reliable real-time communication.
</Admonition>

```mermaid
graph TB
    subgraph "Docker Network: app-network"
        subgraph "Archivemind-Server"
            API[FastAPI<br/>:8080]
            SIO[Socket.IO<br/>:8080]
        end
        
        subgraph "Archivemind-mcp"
            MCP[MCP Server<br/>:8051]
        end
        
        subgraph "Archivemind-agents"
...
|---------|----------------|----------------|-----------|---------|
| Frontend | Archivemind-frontend | 5173 | 3737 | React UI |
| Server | Archivemind-Server | 8080 | 8080 | FastAPI + Socket.IO |
| MCP Server | Archivemind-mcp | 8051 | 8051 | AI Agent connections |
| AI Agents | Archivemind-agents | 8052 | 8052 | AI processing service |
| Documentation | Archivemind-docs | 80 | 3838 | This documentation |
...
Archivemind includes a unified logging system that works seamlessly with or without Logfire:
...
Archivemind supports multiple LLM providers through the Settings UI:
...
Archivemind uses Socket.IO for all real-time features. The default configuration works out of the box:
...
- Frontend → Server: `http://Archivemind-Server:8080`
- API → MCP: `http://Archivemind-mcp:8051`
- API → Agents: `http://Archivemind-agents:8052`
...
proxy: {
  '/api': {
    target: 'http://Archivemind-Server:8080',
    ws: true
  }
}
```
...
- **Location**: `Archivemind-ui-main/src/services/serverHealthService.ts`
...
docker compose restart Archivemind-server

# Rebuild after code changes
docker compose build Archivemind-server
docker compose up -d Archivemind-server

# View real-time logs for a service
docker compose logs -f Archivemind-server
...
Archivemind includes automatic service discovery that works across environments:
...
# Docker: http://Archivemind-Server:8080
