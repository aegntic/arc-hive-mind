---
title: Deployment
description: Quick start guide for running Archivemind
sidebar_position: 10
---
...
Archivemind automatically includes Hot Module Reload for both Python server and React UI. Code changes are reflected immediately without container rebuilds.
</Admonition>
...
1. **Monitor Logs**: Watch for reload messages
   ```bash
   docker logs -f Archivemind-Server
   ```
...
```bash
docker-compose restart Archivemind-mcp
```
</Admonition>
...
graph LR
    ...
    subgraph "Docker Containers"
        Frontend[Archivemind-frontend<br/>React UI]
        API[Archivemind-Server<br/>FastAPI + Socket.IO]
        MCP[Archivemind-mcp<br/>MCP Server]
        Agents[Archivemind-agents<br/>AI Processing]
        Docs[Archivemind-docs<br/>Documentation]
    end
...
# Restart a service
docker-compose restart Archivemind-frontend
```
...
# View service logs
docker logs Archivemind-server
docker logs Archivemind-mcp
docker logs Archivemind-agents
...
<Admonition type="info" title="Scaling Archivemind">
...
Your Archivemind instance is now running. Next:
