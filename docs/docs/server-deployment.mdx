...
<div className="hero hero--primary">
  <div className="container">
    <h2 className="hero__subtitle">
      Deploy Archivemind with Docker: **Production-ready microservices** with health checks, scaling, and monitoring
    </h2>
  </div>
</div>

This guide covers deploying Archivemind's server architecture using Docker and Docker Compose, including configuration, scaling, and production best practices.
...
├── Archivemind-ui-main/              # React frontend application
...
services:
  ...
  Archivemind-server:
    ...
    container_name: Archivemind-Server
    ports:
      - "8080:8080"
    environment:
  ...
  Archivemind-agents:
    ...
    container_name: Archivemind-agents
    ports:
      - "8052:8052"
    environment:
      - SUPABASE_URL=${SUPABASE_URL}
  ...
  # Frontend
  frontend:
    build: ./Archivemind-ui-main
    container_name: Archivemind-frontend
    ports:
      - "3737:5173"
    environment:
    ...
    healthcheck:
      ...
      retries: 3
    volumes:
      - ./Archivemind-ui-main/src:/app/src
      - ./Archivemind-ui-main/public:/app/public
    depends_on:
      - Archivemind-server
  ...
  docs:
    ...
    container_name: Archivemind-docs
    ports:
      - "3838:80"
    networks:
      - app-network
...
1. **Clone the repository**
   ```bash
   git clone https://github.com/Archivemind/Archivemind.git
   cd Archivemind
   ```
...
docker compose up -d --scale Archivemind-agents=3

# Scale the Server service for more concurrent requests
docker compose up -d --scale Archivemind-server=2
...
docker inspect Archivemind-server | grep -A 5 Health
...
docker exec Archivemind-server python -c "
...
# Push to registry
docker tag Archivemind-server:latest your-registry/Archivemind-server:v1.0.0
docker push your-registry/Archivemind-server:v1.0.0
```
...
# Deploy stack
docker stack deploy -c docker-compose.yml Archivemind
```
