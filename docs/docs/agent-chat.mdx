...
```tsx
import { ArchivemindChatPanel } from '@/components/layouts/ArchivemindChatPanel';
...
async def process_agent_response(session_id: str, message: str, context: dict):
    ...
    async with httpx.AsyncClient() as client:
        async with client.stream(
            "POST",
            f"http://Archivemind-agents:8052/agents/rag/stream",
            json={
                "prompt": message,
                "context": context
            }
...
- **Context**: `{ match_count: 5, source_filter: "docs.Archivemind.com" }`
...
AGENT_SERVICE_URL=http://Archivemind-agents:8052
...
- Verify server logs: `docker logs Archivemind-server`
