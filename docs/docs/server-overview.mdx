...
The Server service is the core of Archivemind, containing all business logic, services, and data operations.
...
```yaml
services:
  Archivemind-server:     # FastAPI + Socket.IO (port 8080)
  Archivemind-mcp:        # MCP Server (port 8051)
  Archivemind-agents:     # AI Agents (port 8052)
  Archivemind-frontend:   # React UI (port 3737)
  Archivemind-docs:       # Documentation (port 3838)
```
...
AGENTS_BASE_URL=http://Archivemind-agents:8052
```
...
1. **Clone repository**
   ```bash
   git clone https://github.com/ArchivemindInnovations/Archivemind.git
   cd Archivemind
   ```
