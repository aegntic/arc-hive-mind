...
The MCP service provides a Model Context Protocol interface for AI clients to access Archivemind's functionality via HTTP.
...
Archivemind MCP server uses **SSE (Server-Sent Events) transport only**.
