...
These endpoints are meant to be called only by other services in the ArchiveMind system,
...
ALLOWED_INTERNAL_IPS = [
    ...
    "archivemind-agents",  # Docker service name
    "archivemind-mcp",  # Docker service name
]
...
async def get_agent_credentials(request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        credentials = {
            ...
            # MCP endpoint
            "MCP_SERVICE_URL": f"http://archivemind-mcp:{os.getenv('ARCHIVEMIND_MCP_PORT')}",
            # Additional settings
